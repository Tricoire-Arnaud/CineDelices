...
const recipeController = {
  ...
  getEditRecipeForm: async (req, res) => {
    try {
      ...
      console.log("Données complètes de la recette:", {
        id: recipe.id_recette,
        titre: recipe.titre,
        image: recipe.image,
        chemin_complet: recipe.image ? `/${recipe.image}` : null,
        chemin_uploads: recipe.image
          ? `/uploads/recipes/${recipe.image}`
          : null,
        filename_only: recipe.image ? recipe.image.split("/").pop() : null,
      });
  ...
  updateRecipe: async (req, res) => {
    try {
      ...
      if (req.file) {
        image = `public/uploads/recipes/${req.file.filename}`;
        console.log("Nouvelle image à sauvegarder:", image);
      }
      console.log("Image actuelle de la recette:", recipe.image);
