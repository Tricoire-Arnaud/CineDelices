...
const validators = {
    ...
    recipeValidation: [
        ...
        body('ingredients')
            ...
            .custom((value) => {
                if (!value.every(ing => ing.nom && ing.quantite)) {
                    throw new Error('Tous les ingrédients doivent avoir un nom et une quantité');
                }
                return true;
            }),
        ...
        body('id_film')
            .isInt()
            .withMessage('Film/Série invalide')
    ],
    ...
    filterValidation: [
        body('category')
            .optional()
            .isIn(['all', 'Entrée', 'Plat principal', 'Dessert', 'Boisson', 'Snack', 'Apéritif'])
            .withMessage('Catégorie invalide'),
    ...
    validate: (req, res, next) => {
        ...
        if (!errors.isEmpty()) {
            ...
            if (req.path.includes('/search') || req.path.includes('/catalogue') || req.path.includes('/films-series')) {
                // Nettoyer les paramètres de recherche si invalides
                if (req.query.queryFilms) {
                    req.query.queryFilms = req.query.queryFilms.slice(0, 50).replace(/[^a-zA-Z0-9\s\-']/g, '');
                }
                if (req.query.queryRecipes) {
                    req.query.queryRecipes = req.query.queryRecipes.slice(0, 50).replace(/[^a-zA-Z0-9\s\-']/g, '');
                }
                return next();
            }
